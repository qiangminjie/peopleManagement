...
do_html_header("近日Offer审批-人力资源管理平台");
?>
    ...
    $(document).ready(function () {
        ...
        $("#search-btn").on("click", function () {
            ...
            $.ajax({
                url: '/includes/interview/ajax_interview_list.php',
                type: 'POST',
                data: {code:7, number: number, name: name, type: type, subtype: subtype, level: level},
                dataType: 'JSON',
                beforeSend: function () {
                    // fade in
                    $(".se-pre-con").fadeIn();
    ...
    function showResult(list) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            html += "<tr>" +
                ...
                "<td nowrap class='text-center'>" + getofferStatus(row.isoffer) + "</td>" +
                "<td nowrap class='text-center'>" +
                "<div class='btn-group'>" +
                "<button class='btn btn-primary dropdown-toggle' data-toggle='dropdown'>操作<span class='caret'></span></button>" +
                "<ul class='dropdown-menu'>" +
                "<li class='staff-update' id = '1' staff_id='"+ row.id +"'><a>初试</a></li>" +
                "<li class='staff-update' id = '2' staff_id='"+ row.id +"'><a>复试</a></li>" +
                "<li class='staff-update' id = '3' staff_id='"+ row.id +"'><a>通过</a></li>" +
                "<li class='staff-update' id = '4' staff_id='"+ row.id +"'><a>拒绝</a></li>" +
                "<li class='staff-update' id = '5' staff_id='"+ row.id +"'><a>发送offer</a></li>" +
                "</ul>" +
                "</div>" +
                "</td>" +
                "</tr>";
        ...
        $(".staff-update").click(function () {
            ...
            var offerStatus = $(this).attr("id");
            $.ajax({
                url: '/includes/interview/ajax_interview_list.php',
                type: 'POST',
                data: {code:6, isoffer:offerStatus ,id:id },
                dataType: 'JSON',

                success: function (json) {
                    alert("邀约成功");
                    window.location.href ="/web/interview/offer_review.php";
                },
                error: function (textStatus, errorThrown) {
                    showSingleButtonNoty("服务器内部错误,请稍后重试. 如果此问题重复出现,请联系网站管理人员", "error", 0);
                    console.error(textStatus);
